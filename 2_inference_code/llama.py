...
if __name__ == "__main__":
    ...
    #  Model
    # ──────────────────────────────────────────────────────────────────────────────
    model_id = "/data/dbravoh/VLM/models/Llama-3.2-11B-Vision-Instruct"

    model = MllamaForConditionalGeneration.from_pretrained(
        model_id,
        torch_dtype=torch.bfloat16,
        #device_map="cuda:0", # Asigna explícitamente a la primera GPU
        device_map={"": "cuda:1"}
        #device_map="auto",
    )
    ...
    #  Paths and Experiment Selection
    # ──────────────────────────────────────────────────────────────────────────────
    dataset_dir = "/data/dbravoh/VLM/1_dataset_guide/questions/original"

    RESULTS_ROOT = '/data/dbravoh/VLM/1_dataset_guide/questions/original/results'  # path for results directory
        ...
        for sub_experiment, data in experiment_plan.items():
            ...
            for i in range(3):  # how many runs ?
                ...
                save_name = os.path.join(
                    RESULTS_ROOT, exp, "llama", results_file_name)
