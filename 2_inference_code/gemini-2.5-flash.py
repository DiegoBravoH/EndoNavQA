"""
Script for Running Visual QA Experiments with Google AI Studio.

Overview:
This script processes images and associated questions, sends them via the API to Gemini model,
...
try:
    # Asegúrate de que tu .env tenga la GOOGLE_API_KEY
...
def make_gemini_api_call(questions_data, base64_image, additional_question):
    """
    Sends a structured API call to Google's Gemini model with a medical image
    ...
    Args:
        ...
        base64_image (str): A base64-encoded PNG image.
    ...
    # El prompt textual es idéntico, ya que las instrucciones son para el modelo
    prompt = (
        "The image is a high-definition frame from an upper gastrointestinal endoscopy (gastroscopy). "
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    #  Construcción de la solicitud para Gemini
    # ──────────────────────────────────────────────────────────────────────────────
    
    # 1. Configurar el modelo
    # Puedes usar "gemini-1.5-pro-latest" (más potente) o "gemini-1.5-flash-latest" (más rápido)
    model = genai.GenerativeModel("gemini-2.5-flash")   #gemini-2.5-pro , gemini-2.5-flash
    # 2. Configurar la generación (equivalente a temperature=0)
    generation_config = genai.types.GenerationConfig(
        temperature=0
    )

    # 3. Preparar la imagen para la API de Gemini
    # La API de Gemini acepta datos base64 directamente
    image_part = {
        "mime_type": "image/png",  # Coincide con el formato de encode_image_from_bytes
        "data": base64_image
    }

    # 4. Combinar el prompt de texto y la imagen
    ...
    try:
        ...
        print(f"\nError en la API de Gemini: {e}")
        # Esto puede ocurrir por bloqueos de seguridad (safety settings) u otros errores.
        model_answer = f"API_ERROR: {str(e)}"
...
if __name__ == "__main__":
    ...
    dataset_dir = "/data/dbravoh/VLM/1_dataset_guide/questions/rotation"

    RESULTS_ROOT = "/data/dbravoh/VLM/1_dataset_guide/questions/rotation/results"  # path for results directory

    experiments = ['Level1','Level2','Level3']   # select the experiments here: 'Level1', 'Level2', 'Level3', 'AS'
    ...
    for exp in experiments:
        ...
        for sub_experiment, data in experiment_plan.items():
            ...
            for i in range(1):  # how many runs ?
                start_time = time.time()

                dataset_results = []

                for image in tqdm(png_images):
                    ...
                    results_call = make_gemini_api_call(
                ...
                save_name = os.path.join(
                    RESULTS_ROOT, exp, "gemini-2.5-flash", results_file_name)
