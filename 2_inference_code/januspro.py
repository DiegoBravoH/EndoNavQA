...
def make_model_call(model, questions_data, original_image_path, additional_question):
    ...
    # ---------------------------------------------------------------------
    # Optimized inference (faster GPU throughput)
    # ---------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    MODEL_SIZE = '7B'  # 1B, 7B
    model_path = "/data/dbravoh/VLM/models/Janus-Pro-7B"
    # ---------------------------------------------------------------------
    # Optimized model loading (faster startup, full GPU utilization)
    # ---------------------------------------------------------------------
    ...
    vl_gpt: MultiModalityCausalLM = AutoModelForCausalLM.from_pretrained(
        model_path,
        torch_dtype=torch.bfloat16,    # carga directamente en BF16
        device_map={"": "cuda"},       # carga todo en GPU
        low_cpu_mem_usage=True,        # streaming loading
        ...
        for sub_experiment, data in experiment_plan.items():
            ...
            for i in range(3):  # how many runs ?
                ...
                save_name = os.path.join(
                    RESULTS_ROOT, exp, "januspro",results_file_name)
