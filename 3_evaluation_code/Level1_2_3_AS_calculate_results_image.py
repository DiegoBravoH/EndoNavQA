...
1. Place the model output files (`*_run_0.json`, `*_run_1.json`, `*_run_2.json`) in a folder called `answers/`.
    The pathes should look like this: (alternatively, you can change the paths in the main method of the code)
      Base
      ├── Level1_2_3_AS_calculate_results_image.py
      └── answers/
          ├── <base>_run_0.json
          ├── <base>_run_1.json
          └── <base>_run_2.json
...
def parse_model_answer(ans: str, q: str, prompt: str):
    import re

    # --- Protección contra respuestas vacías o None ---
    if ans is None:
        return None, None
    ...
    # --- Ahora sí sigue la lógica original ---
    def sdigit(s: str):
...
if __name__ == "__main__":
    ...
    # ── Paths  ─────────────────────────────────────
    answer_files_dir = "/data/dbravoh/VLM/1_dataset_guide/questions/QA original/results/AS/gemini-2.5-flash" #path result
    ...
    #  {
    #         "file_name": "007efc9b-949c-4e44-9da1-3c7dba513ceb_above_Greater_Lesser.jpg",
    #         "results_call": [
    #             {
    #                 "question": "In this image, is the red dot above the blue dot?",
    #                 "model_answer": "0",
    #                 "expected_answer": 0,
    #                 "entire_prompt": "The image is a high-definition frame from an upper gastrointestinal endoscopy (gastroscopy).
    ...
    #                                   \nExample:\nQ: Is the red dot below to the red dot? 
    ...
    for base, run_paths in grouped.items():
        ...
        out_path = os.path.join(answer_files_dir, out_name)
        wb.save(out_path)
